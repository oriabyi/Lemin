...
/*   Updated: 2018/11/01 14:43:47 by ariabyi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_ways			*get_all_ways(t_room *cur, t_room *end, t_way **phial)
{
	t_ways		*ret;
	...
	if (*phial)
		ret = get_all_ways(get_next_way(phial), end, phial);
	return (ret);
}
...
void			get_ways(t_lemin *lemin)
{
	...
	while (1)
	{
		res = get_all_ways(lemin->start, lemin->end, &phial);
		free_phial(phial);
		phial = NULL;
		set_default(lemin);
		if (!res)
